...
public class HouseOwnerController {
     ...
     */
    @GetMapping("/page")
    @Operation(summary = "分页查询房屋业主关联信息", description = "根据条件分页查询房屋业主关联信息列表")
    ...
    public Result page(@Parameter(description = "当前页码") @RequestParam(defaultValue = "1") Integer pageNum,
        ...
        IPage<HouseOwner> page = new Page<>(pageNum, pageSize);
        IPage<HouseOwner> result = houseOwnerService.selectHouseOwnerPage(page, houseOwner);
     ...
     */
    @GetMapping("/{id}")
    @Operation(summary = "根据ID查询房屋业主关联信息", description = "根据ID查询指定房屋业主关联信息")
    @ApiOperationSupport(order = 2, author = "开发团队")
    @SecurityRequirement(name = "Authorization")
    public Result getById(@Parameter(name = "id", description = "房屋业主关联ID", required = true, example = "1") @PathVariable Long id) {
        HouseOwner houseOwner = houseOwnerService.getById(id);
        return Result.ok().put("data", houseOwner);
    }
